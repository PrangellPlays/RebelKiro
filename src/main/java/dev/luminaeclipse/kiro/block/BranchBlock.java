...
public class BranchBlock extends ConnectingBlock implements Waterloggable {
    ...
    private final @Nullable BranchBlock strippedBlock;
    ...
    public BranchBlock(AbstractBlock.Settings settings, @Nullable BranchBlock stripped) {
        ...
        this.setDefaultState((BlockState)((BlockState)((BlockState)((BlockState)((BlockState)((BlockState)((BlockState)super.getDefaultState().with(WATERLOGGED, false)).with(NORTH, false)).with(EAST, false)).with(SOUTH, false)).with(WEST, false)).with(UP, false)).with(DOWN, false));
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (stack.getItem() instanceof AxeItem && this.strippedBlock != null) {
            if (!world.isClient) {
                world.playSound((PlayerEntity)null, pos, SoundEvents.ITEM_AXE_STRIP, SoundCategory.BLOCKS, 1.0F, 1.0F);
            }
    ...
    public @Nullable BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        return (BlockState)((BlockState)getConnectedState(this.getDefaultState(), pos, world).with(WATERLOGGED, bl)).with((Property)FACING_PROPERTIES.get(ctx.getSide().getOpposite()), true);
    }
