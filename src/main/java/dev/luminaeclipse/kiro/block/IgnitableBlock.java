...
public abstract class IgnitableBlock extends Block implements Waterloggable {
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        if ((Boolean)state.get(LIT)) {
            for(Vec3d offset : this.getParticlePositions(state)) {
                this.spawnParticles(world, state, offset.add((double)pos.getX() + (double)0.5F, (double)pos.getY(), (double)pos.getZ() + (double)0.5F), random);
            }
        }
    ...
    private void spawnParticles(World world, BlockState state, Vec3d vec3d, Random random) {
        ...
        while(var6.hasNext()) {
            Triplet<DefaultParticleType, Float, Float> particle = (Triplet)var6.next();
            if (f < (Float)particle.getB()) {
                world.addParticle((ParticleEffect)particle.getA(), vec3d.x, vec3d.y, vec3d.z, (double)0.0F, (double)(Float)particle.getC(), (double)0.0F);
            }
        }

        if (f < 0.17F) {
            world.playSound(vec3d.x + (double)0.5F, vec3d.y + (double)0.5F, vec3d.z + (double)0.5F, this.getSound(state), SoundCategory.BLOCKS, 1.0F + random.nextFloat(), random.nextFloat() * 0.7F + 0.3F, false);
        }
    ...
    protected abstract ImmutableList<Triplet<DefaultParticleType, Float, Float>> getParticles(BlockState var1);
        ...
        if (!isLit(state)) {
            if (canBeLit(state)) {
                ...
                if (stack.getItem() instanceof FlintAndSteelItem || EnchantmentHelper.get(stack).containsKey(Enchantments.FIRE_ASPECT)) {
                    setLit(world, state, pos, true);
                    world.playSound((PlayerEntity)null, pos, SoundEvents.ITEM_FLINTANDSTEEL_USE, SoundCategory.BLOCKS, 1.0F, world.getRandom().nextFloat() * 0.4F + 0.8F);
                    if (!player.isCreative()) {
    ...
    public boolean tryFillWithFluid(WorldAccess world, BlockPos pos, BlockState state, FluidState fluidState) {
        if (!(Boolean)state.get(WATERLOGGED) && fluidState.getFluid() == Fluids.WATER) {
            ...
            if ((Boolean)state.get(LIT)) {
                this.extinguish((PlayerEntity)null, blockState, world, pos);
            } else {
    ...
    public @Nullable BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        return fluidState.getFluid().equals(Fluids.WATER) ? (BlockState)((BlockState)this.getDefaultState().with(WATERLOGGED, true)).with(LIT, false) : this.getDefaultState();
    }

    public void extinguish(@Nullable PlayerEntity player, BlockState state, WorldAccess world, BlockPos pos) {
        setLit(world, state, pos, false);
        world.playSound((PlayerEntity)null, pos, this.getExtinguishedSound(), SoundCategory.BLOCKS, 1.0F, 1.0F);
